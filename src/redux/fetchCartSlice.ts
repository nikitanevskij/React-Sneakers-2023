...
export const fetchPOSTOrderSneakers = createAsyncThunk<number, undefined, { state: RootState }>(
  'sneakers/fetchPOSTOrderSneakers',
  async (_, { getState }) => {
...
export const fetchCartSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchPOSTOrderSneakers.fulfilled, (state, action) => {
      state.orderId = action.payload;
      state.totalPrice = 0;
    });
    builder.addCase(fetchPOSTOrderSneakers.rejected, () => {
